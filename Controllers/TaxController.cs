...
namespace MyCvMvcApp.Controllers
{
    public class TaxController : Controller
    {
        ...
        public IActionResult Calculate(decimal income)
        {
            ...
            if (income <= 110000)
                resModel.Tax = income * 0.15m;
            else if (income <= 230000)
                resModel.Tax = (110000 * 0.15m) + ((income - 110000) * 0.20m);
            else if (income <= 580000)
                resModel.Tax = (110000 * 0.15m) + (120000 * 0.20m) + ((income - 230000) * 0.27m);
            else
                resModel.Tax = (110000 * 0.15m) + (120000 * 0.20m) + (350000 * 0.27m) + ((income - 580000) * 0.35m);

            ViewBag.Tax = tax;
            ViewBag.Income = income;

            return View("Result", resModel);
        }
    }
}
